...
if (invitorEmail === inviteeEmail) {
    ...
    return res.status(404).json({ statusCode: 404,message: "You can not invite yourself." });            
}
...
if (!userInvitor || !userInvitor.length) {
    //no invotor found in the system
    return res.status(404).json({ statusCode: 404,message: "Not be able to find the invitor in the system." });            
}

if (!userInvitee || !userInvitee.length) {
    //no invotor found in the system
    return res.status(404).json({ statusCode: 404,message: "Not be able to find the invitee in the system." });            
}
...
return res.status(200).json({ statusCode: 200, message: "Invitation Created for, "+ userInvitor[0].name, body:invitationPlayers });
}));

//api to send gaming invitation information for double players
router.post('/inviteDoublePlayer', asyncHandler(async(req, res) =>{
    let invitorEmail = req.body.invitorEmail;
    ...
    let emailArr = [invitorEmail,invitorPartnerEmail,inviteePlayer1Email,inviteePlayer2Email];
    if(unique(emailArr).length !== 4){
        return res.status(404).json({ statusCode: 404,message: "Not be able to invite same player to be your partner or opponent at the same time." });            
    ...
    if (!userInvitorPartner || !userInvitorPartner.length) {
        //no invotor found in the system
        return res.status(404).json({ statusCode: 404,message: "Not be able to find the this invitor's partner in the system." });            
    }
